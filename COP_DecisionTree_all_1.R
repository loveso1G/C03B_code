...
#rawdata=read.csv("Two_imf6.csv",header=T)

#rawdata=read.csv("Two_all_R.csv",header=T)
#rawdata=read.csv("Two_raw_R.csv",header=T)
#rawdata=read.csv("Two_imf1_R.csv",header=T)
#rawdata=read.csv("Two_imf2_R.csv",header=T)
#rawdata=read.csv("Two_imf3_R.csv",header=T)
#rawdata=read.csv("Two_imf4_R.csv",header=T)
#rawdata=read.csv("Two_imf5_R.csv",header=T)
#rawdata=read.csv("Two_imf6_R.csv",header=T)

#rawdata=read.csv("Two_all_L.csv",header=T)
#rawdata=read.csv("Two_raw_L.csv",header=T)
#rawdata=read.csv("Two_imf1_L.csv",header=T)
#rawdata=read.csv("Two_imf2_L.csv",header=T)
#rawdata=read.csv("Two_imf3_L.csv",header=T)
#rawdata=read.csv("Two_imf4_L.csv",header=T)
#rawdata=read.csv("Two_imf5_L.csv",header=T)
#rawdata=read.csv("Two_imf6_L.csv",header=T)

#rawdata=read.csv("ALL_1.csv",header=T)
#rawdata=read.csv("ALL_raw.csv",header=T)
#rawdata=read.csv("ALL_imf1.csv",header=T)
#rawdata=read.csv("ALL_imf2.csv",header=T)
#rawdata=read.csv("ALL_imf3.csv",header=T)
#rawdata=read.csv("ALL_imf4.csv",header=T)
#rawdata=read.csv("ALL_imf5.csv",header=T)
rawdata=read.csv("ALL_imf6.csv",header=T)


#F_index=c(2,72);  #CF#for single,replace 3¡A3 to...72)
F_index=c(2:12); #C4#for single,replace 3¡A3 to...72)
dim(rawdata)
# data separation
np=283 # 20% testing data#The np value can be changed according to requirements #C4's np value is 177
...
np=283 # 20%
    ...
    np283=c(acc.train.all,sen.train.all,spe.train.all,acc.test.all,sen.test.all,spe.test.all);
print(np283)
    np283.sd=c(acc.train.sd,sen.train.sd,spe.train.sd,acc.test.sd,sen.test.sd,spe.test.sd);
print(np283.sd) 


 write.table(Result_all,file="all_imf6_result.csv",sep=",",col.names=FALSE,row.names=FALSE,quote=TRUE) # edit files names#Just change cf_6
